...
router.post("/login3", async (req, res) => {
  try {
    ...
    // Save minimal user info into server session so other pages can verify session
    try {
      req.session.user = userData;
      // ensure session is saved before responding
      req.session.save(err => {
        if (err) console.error('[LOGIN3] session save error:', err);
      ...
      console.error('[LOGIN3] Unable to set session:', sessErr);
